...
const imageAssetModules = import.meta.glob("./assets/*.png", {
  eager: true,
  import: "default"
...
export async function run(root: HTMLElement): Promise<void> {
  ...
  const stimPairs = buildStimPairs(normalizeImportedAssets(imageAssetModules));
  ...
  await mountTaskApp({
    root,
    task_id: "H000009-prl",
    task_name: "Probabilistic Reversal Learning (PRL) Task",
    task_description: "HTML preview aligned to local psyflow PRL task procedure and parameters.",
    settings,
    subInfo,
    stimBank,
    buildTrials: (): CompiledTrial[] => {
      ...
      for (let blockIndex = 0; blockIndex < totalBlocks; blockIndex += 1) {
        ...
        const pair = getBlockStimPair(stimPairs, blockIndex);
          ...
          [
            (_snapshot: TrialSnapshot, runtime: RuntimeView) =>
              stimBank.get_and_format("good_bye", {
                total_score: runtime.sumReducedField("choice_delta")
              })
          ]
        )
      );
